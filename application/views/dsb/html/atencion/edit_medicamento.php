...
<script type="text/javascript">
  ...
  <body>
    ...
    <nav class="navbar navbar-fixed-top" style="background-color:#438eb9 !important; border-width: 0 0 0 px !important;">
      <div class="container">
        <div class="navbar-header">
          ...
          <a class="navbar-brand" style="color:white; " href="#">Editar Tratamiento</a>
        </div>
    ...
    <div class="jumbotron">
      <div class="container">        
		<form id="form2" name="form2" role="form" action="<?=base_url()?>index.php/guardaEditMed" method="post">	
			...
			<div id="grupo5">
				...
				<div class="form-group col-md-6">						        
					...
					<input  type="hidden" name="medicamento_temp" id="medicamento_temp" value="<?php echo $medicamento_temp;?>">
					<input  type="hidden" name="idtratamiento" id="idtratamiento" value="<?php echo $idtratamiento;?>">
					
					<h4 style="color:#033766;">Tratamiento a modificar</h4>
					<div id="messageAtention">
						<h5>Todo tratamiento indicado como tratamiento secundario deberán ser asumidos por el paciente.</h5>
					</div>
					...
					<div id='itemRows2'>
					    <table class='table'>
							...
							<tr class='fila-base'>
							<td>
								<select class='form-control' name='medi' id='medi' style='width:15.5em;'> 
									<?php 
									if (count($medicamento)) {
										foreach ($medicamento as $list) {
											if ($list['idmedicamento']==$idmedicamento){
																	
												echo "<option value='". $list['idmedicamento']."'selected>" . $list['nombre_med']." / ".$list['presentacion_med']."</option>";	
											}else{

												echo "<option value='". $list['idmedicamento']."'>" . $list['nombre_med']." / ".$list['presentacion_med']."</option>";			
												}
							...
							<td>
								<input type='text' class='form-control' name='cant' id='cant' style='width:8em;' value="<?php echo $cantidad_trat; ?>"><br><br>
								</td> 
							<td>
								<input type='text' class='form-control' name='dosis' id='dosis' style='width:15.5em;' value="<?php echo $dosis_trat; ?>"><br><br>
							</td>		
							<td> 
								<select class='form-control' name='tipo' id='tipo' style='width:8.5em;'>
									<?php  if ($tipo_tratamiento==3){?>
								<option value="<?php echo $tipo_tratamiento; ?>">No Cubierto</option>
								<option value="1">Cubierto</option>
									<?php } else { ?>
								<option value="<?php echo $tipo_tratamiento; ?>">Cubierto</option>
								<option value="3">No Cubierto</option>
									<?php } ?>
	...
	<script type="text/javascript">
		...
		$('#medi').bind('change click keyup', function() {
		  $('#medicamento_temp').val($("#medi option:selected").text());
		});
