						...
						<div class="row">
							<div class="col-xs-9">
								...
								<form class="form-horizontal" name="form1" id="form1" role="form" method="post">
									<input type="hidden" name="idclienteempresa" value="<?=$idclienteempresa?>">
									...
									<div class="form-group">
										<div class="col-sm-9" id="serieActual" name="serieActual">
											<input type="text" class="form-control" name="serieGen" id="serieGen" value="" readonly>
										</div>
									</div>							
									<div class="clearfix form-actions">
										<div class="col-md-offset-3 col-md-5">
											<button class="btn btn-info" type="submit" id="guardar" name="guardar" value="guardar">
		...
		<script>
   			$(document).ready(function() {
   				$('select[name=tipoDoc]').change(function() {
		            $.ajax({
		                url: "<?= BASE_URL()?>index.php/Series_cnt/correlativoSerie",
		                type: 'POST',
		                dataType: 'json',
		                data: $("#form1").serialize(),
		                success: function(data)
		                {   
		                    $("#serieActual").html(null); 
							$("#serieActual").html(data);
   				...
   				$("#form1").submit(function(e) {
    				e.preventDefault();
		            $.ajax({
		                url: "<?= BASE_URL()?>index.php/Series_cnt/reg_serie",
		                type: 'POST',
		                dataType: 'json',
		                data: $("#form1").serialize(),
		                complete:function(){
		                    alert('Se agregó la serie.');
