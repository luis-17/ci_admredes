 ...
 $this->load->database();
	...
	function getPlanes2($id){
 	...
 	function certificado($data){
 		$this->db->select("cert_id, cert_num, p.num_afiliados");
    ...
    $this->db->select("cert_id, c.cont_id, cont_numDoc, cont_nom1, cont_nom2, cont_ape1, cont_ape2, cont_tipoDoc, cont_direcc, cont_telf, cont_email, coalesce(SUBSTR(cont_ubg, 15,2),'') as dep, coalesce(SUBSTR(cont_ubg, 15,4),'') as prov, coalesce(SUBSTR(cont_ubg, 15,6),'') as dist");
	...
	$this->db->select("a.aseg_id,aseg_numDoc, concat(aseg_ape1,' ', aseg_ape2,' ', aseg_nom1,' ', COALESCE(aseg_nom2,'')) as asegurado");
	...
	function in_certase($id,$cert){
		$array = array(
			'cert_id' => $cert,
			'aseg_id' => $id,
			'cert_estado' => 1,
			'cert_iniVig' => "now()",
			'cert_finVig' => "(SELECT cert_finVig from certificado WHERE cert_id=".$cert.")"
			);
	...
	function num_aseg($data){
		$this->db->select("count(a.aseg_id)as cant");
		$this->db->from('asegurado a ');
		$this->db->join("certificado_asegurado ca","a.aseg_id=ca.aseg_id");
		$this->db->join("certificado c","c.cert_id=ca.cert_id");
		$this->db->join("contratante co","c.cont_id=co.cont_id");
